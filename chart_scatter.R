...
chartScatter <- function(aX, aY, aColor = 1, aSavePlot=F,
  ...
  if (aSavePlot) {
    png(file=aSavePlotFilename, width=aWidth, height=aHeight, pointsize=16)
    par(mar=c(12.1, 7.1, 4.1, 14.1), xpd=TRUE, mgp = c(4, 1, 0)) #mgp adjusts the label positions
  } else {
    par(mar=c(12.1, 7.1, 4.1, 12.1), xpd=TRUE, mgp = c(4, 1, 0)) #mgp adjusts the label positions
  }
  
  if (aShowFittedLine) {
    scatter.smooth(x=aX, y=aY, 
  ...
  #Plot subset points
  if (aShowSubset) {
    points(x=aSubsetX, y=aSubsetY, col="black",
           bg=c(1:length(aSubsetX)), pch=21, cex=1.9)
  }
  #Show legend
  if (aShowLegend) {
    legend("topright",inset=c(-0.20,0), legend = 
             aLegend, 
